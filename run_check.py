...
if os_type == 'Darwin':  # Mac OS
    print("\033[0;31m")  # ANSI escape code for red
    print("################################################################################")
    print("# WARNING: Macs are not permitted in this program.                             #")
    print("# Only Windows and Linux host machines are allowed.                            #")
    print("################################################################################")
    ...
    print_table_results("2GHz+ Processor", "cat /proc/cpuinfo | awk '/cpu MHz/'" | head -n 1, comparator=lambda x, y: float(x.split(':')[1].strip()) >= y, required_value=2000.0)  # Comparing in MHz
    print_delimiter()
