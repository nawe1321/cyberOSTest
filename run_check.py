...
if os_type == 'Darwin':  # Mac OS
    ...
    print_table_results("2GHz+ Processor", "cat /proc/cpuinfo | head -n 1| awk '/cpu MHz/'", comparator=lambda x, y: float(x.split(':')[1].strip()) >= y, required_value=2000.0)  # Comparing in MHz
    print_delimiter()
